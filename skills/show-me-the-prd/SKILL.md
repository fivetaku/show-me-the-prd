...
**수정 루프**: "수정할 부분이 있어요" 선택 시 → 사용자 피드백을 받아 데이터 모델 수정 → Step 3-1부터 재출력 → Step 3-2 재확인. "좋아요" 선택될 때까지 반복.
...
**수정 루프**: "순서 변경" 또는 "합치기/나누기" 선택 시 → 사용자 피드백 반영하여 Phase 재구성 → Step 4-1부터 재출력 → Step 4-2 재확인.
...
- 비교 테이블은 AskUserQuestion 호출 전에 일반 텍스트로 출력한다 (Step 5-1)
- 인증 방식(Q2)은 정적 JSON (그대로 사용)
