...
**반드시 AskUserQuestion 도구를 호출한다** (아래를 도구 파라미터로 변환하여 호출. 텍스트 출력 금지. 최대 4개 질문 묶기):
...
**반드시 AskUserQuestion 도구를 호출한다** (아래를 도구 파라미터로 변환하여 호출. 텍스트 출력 금지. markdown preview 활용):

```
Q1. "데이터 구조 이렇게 잡았는데 맞아요?" (header: "데이터")
    ⚡ markdown preview 사용 -- 위의 텍스트 설명을 시각적 다이어그램으로 변환하여 옵션에 첨부
    options:
    - "좋아요, 이대로 갈게요 (추천)" -- 일반적인 구조라 확장하기 좋아요
      markdown: |
        (엔티티 관계도 + 핵심 필드 테이블을 동적 생성)
        예시 형식:
        ```
        ...
        | 엔티티 | 핵심 필드 | 설명 |
        |--------|----------|------|
        | User | email, name | 사용자 정보 |
        | Todo | title, dueDate, priority | 할 일 항목 |
        | Category | name, color | 분류 |
    - "수정할 부분이 있어요" -- 어떤 부분을 바꾸고 싶은지 알려주세요
      markdown: (동일 관계도 -- 수정 가능한 포인트를 ← 로 표시)
    - "잘 모르겠어요" -- 추천대로 갈게요
...
Q1. "이 Phase 분리가 괜찮아요?" (header: "Phase")
    ⚡ markdown preview 사용 -- Phase별 기능을 체크리스트로 시각화하여 옵션에 첨부
    options:
    - "좋아요, 이대로 갈게요 (추천)" -- 보편적인 순서예요
      markdown: |
        (Phase별 기능 체크리스트를 동적 생성)
        예시 형식:
        ## Phase 1: MVP
        - [x] 회원가입 / 로그인
        - [x] Todo CRUD (추가, 수정, 삭제, 완료)
        - [x] 기본 필터 (전체 / 완료 / 미완료)
...
Q1. "코드를 만들 때 어떤 도구를 쓸지 정해야 해요" (header: "기술 스택")
    ⚡ markdown preview 사용 -- 각 스택의 프로젝트 구조 + 비교표를 시각화하여 옵션에 첨부
    -> 리서치 기반 2-3개 선택지
    -> 각 옵션 description에 반드시 포함:
       ...
       - 확장성
    -> 각 옵션 markdown에 포함:
       - 프로젝트 디렉토리 트리 (실제 폴더 구조)
       - 비교 테이블 (비용, 호환성, 난이도)
    예시:
      option: "Next.js + Supabase (추천)"
      markdown: |
        ```
        my-app/
        ├── src/
        ...
        | 항목 | 내용 |
        |------|------|
        | 무료 시작 | Vercel + Supabase 무료 티어 |
        | AI 호환성 | 높음 (Claude가 잘 다룸) |
